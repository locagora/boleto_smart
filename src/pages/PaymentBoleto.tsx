...
import { RefreshCw, Loader2, ExternalLink, DollarSign, Users, AlertCircle, CheckCircle, Search, Filter, FileText } from "lucide-react";
...
const PaymentBoleto = () => {
  ...
  const [payments, setPayments] = useState<PaymentBoletoData[]>([]);
  ...
  const loadPayments = async () => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: error.message || "Falha ao consultar 'payment_boleto'",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleRefresh = async (franquia: Franquia) => {
    ...
    toast({
      title: "Processando...",
      description: `Buscando dados de Boletos de ${franquia.nome}`,
    });
    ...
    try {
      const response = await fetch(franquia.webhook, {
        ...
        body: JSON.stringify({ atualiza: "boleto" }),
      });

      const data = await response.json();
      console.log("Webhook Boleto response:", data);

      const paymentsArray = Array.isArray(data) ? data : (data?.data || []);

      const boletoPaymentsRaw = paymentsArray.filter((item: any) => {
        const type = item.billingType || item.billing_type;
        return type && type.toUpperCase() === 'BOLETO';
      });

      console.log(`Webhook retornou ${paymentsArray.length} itens. ${boletoPaymentsRaw.length} identificados como BOLETO.`);

      if (boletoPaymentsRaw.length > 0) {
        const boletoPayments = boletoPaymentsRaw.map((item: any) => ({
          id: item.id,
          ...
          bank_slip_url: item.bankSlipUrl || item.bank_slip_url || null,
        ...
        for (const payment of boletoPayments) {
          ...
          if (error) {
            console.error("Error upserting payment:", error, payment);
            errorCount++;
          } else {
            successCount++;
          }
        ...
        if (successCount > 0) {
          toast({
            ...
            description: `${successCount} boletos atualizados.${errorCount > 0 ? ` ${errorCount} falharam.` : ''}`,
            variant: errorCount > 0 ? "destructive" : "default",
          });
        } else if (errorCount > 0) {
          toast({
            title: "Erro ao salvar",
            description: `Falha ao salvar ${errorCount} boletos. Verifique se a tabela 'payment_boleto' existe e as permissões. Detalhes no console.`,
            variant: "destructive",
          });
        }
      } else {
        toast({
          title: "Aviso",
          description: "Nenhum pagamento de Boleto encontrado no webhook.",
        });
      }
    } catch (error) {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">Pagamentos Boletos</h1>
            <p className="text-muted-foreground">
              Visão geral dos pagamentos via Boleto
            </p>
          </div>
        </div>
          ...
          <Card className="p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                <p className="text-sm text-muted-foreground">Boletos Recebidos</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="p-6">
            <h3 className="text-lg font-semibold text-foreground mb-4">Valores Boletos por Cliente</h3>
            {chartDataByClient.length > 0 ? (
              ...
              <div className="h-64 flex items-center justify-center text-muted-foreground">
                Nenhum pagamento de Boleto encontrado
              </div>
            )}
          </Card>

          <Card className="p-6">
            <h3 className="text-lg font-semibold text-foreground mb-4">Qtd. Boletos por Cliente</h3>
        ...
        {(userRole === 'master_regional' || userRole === 'super_admin') && franquiaChartData.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="p-6">
              <h3 className="text-lg font-semibold text-foreground mb-4">Valores Boletos por Franquia</h3>
              <div className="h-80 overflow-y-auto">
            ...
            <Card className="p-6">
              <h3 className="text-lg font-semibold text-foreground mb-4">Qtd. Boletos por Franquia</h3>
              <div className="h-80 overflow-y-auto">
                <div style={{ height: Math.max(320, franquiaChartData.length * 40) }}>
                  <ResponsiveContainer width="100%" height="100%">
              ...
              <CardContent>
                {franquiaPayments.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    Nenhum pagamento de Boleto encontrado. Clique em "Atualizar" para buscar dados.
                  </p>
                ) : (
